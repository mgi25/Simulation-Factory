...
from .objective_lifecycle import TERMINAL, ObjectiveState, parse_state
...
#: Every field a CEO must state to open a live objective. Absence is a refusal,
#: never a default: a contract that quietly filled in a risk ceiling or an
#: expiry would be authorizing something nobody wrote down.
...
class ObjectiveContract:
    ...
    @classmethod
    def from_mapping(cls, data: Any) -> "ObjectiveContract":
        """Build a contract from a CEO's file, refusing anything incomplete.

        Every field in `REQUIRED_CONTRACT_FIELDS` must be present and non-empty.
        There is no defaulting here on purpose: a missing risk ceiling that
        quietly became LOW, or a missing expiry that quietly became a year out,
        would be live authority nobody authorized. `max_corrections_per_work_order`
        and `forbidden_actions` do have defaults, because the dataclass defaults
        for those are the *narrow* answer - one correction, nothing extra
        forbidden beyond what is always forbidden.
        """
        if not isinstance(data, Mapping):
            raise DelegationError("an objective contract is a mapping")
        missing = [
            name
        ...
        if missing:
            raise DelegationError(
                "the objective contract is incomplete and is refused rather than "
                "filled in. Missing: " + ", ".join(sorted(missing)) + ". Every one "
                "of these is something the CEO has to state; a default here would "
                "be authority nobody wrote down."
            )
        unknown = set(data) - {
        ...
        if unknown:
            raise DelegationError(
                "the objective contract names field(s) this runtime does not "
                "understand: " + ", ".join(sorted(unknown)) + ". A contract with a "
                "field nobody reads is a contract whose author believes something "
                "untrue about what was authorized."
            )
        budget = data["budget"]
        kwargs: dict[str, Any] = {
            ...
            "mode": OperatingMode(str(data["mode"]).strip().lower()),
            "department": str(data["department"]),
            "budget": budget if isinstance(budget, Money) else Money.from_dict(dict(budget)),
            "budget_scope": str(data["budget_scope"]),
        ...
        if "max_corrections_per_work_order" in data:
            kwargs["max_corrections_per_work_order"] = int(
                data["max_corrections_per_work_order"]
            )
        if data.get("reporting_requirements"):
            kwargs["reporting_requirements"] = tuple(data["reporting_requirements"])
        if data.get("notes"):
...
def may_activate(
    ...
    *,
    ...
    `objective_state` and `revoked` are how authority *ends*. An objective that
    reached a terminal state has no further work to authorize, and a revoked
    contract has none either. Both are checked here rather than left to the
    caller, because "the objective finished but the activation object is still
    lying around" is exactly the residual-authority bug this mode must not
    have.
    """
    ...
    if revoked:
        return ModeVerdict(
            ...
            may_activate=False,
            reason=(
                "the contract was revoked; a revoked contract authorizes nothing "
                "from the moment it is revoked, whatever else is still true of it"
            ),
        )
    if objective_state is not None:
        state = parse_state(objective_state, "objective_state")
        if state in TERMINAL:
            return ModeVerdict(
                ...
                may_activate=False,
                reason=(
                    f"the objective is {state.value} and has finished. A terminal "
                    "objective has no remaining work to authorize, so no live "
                    "authority survives it."
                ),
            )
    enabled = {
