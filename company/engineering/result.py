...
class EngineeringResult:
    ...
    def __post_init__(self) -> None:
        ...
        for name in ("developer_attempts", "max_developer_attempts", "attempts_remaining"):
            value = getattr(self, name)
            if isinstance(value, bool) or not isinstance(value, int):
                raise EngineeringError(f"result.{name} must be an integer")
            if value < 0:
                raise EngineeringError(f"result.{name} must be non-negative")
        if self.max_developer_attempts > 0 and (
            self.developer_attempts + self.attempts_remaining != self.max_developer_attempts
        ):
            raise EngineeringError(
                f"result.developer_attempts ({self.developer_attempts}) + "
                f"result.attempts_remaining ({self.attempts_remaining}) must equal "
                f"result.max_developer_attempts ({self.max_developer_attempts})"
            )
    ...
    def render_text(self) -> str:
        ...
        lines.append(
            ...
            f"{self.attempts_remaining} remaining (ceiling {self.max_developer_attempts})"
        )
