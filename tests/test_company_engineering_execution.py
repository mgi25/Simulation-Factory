...
def test_a_ceo_objective_produces_a_bounded_work_order(tmp_path):
    ...
    # requires tests to pass has to allow writing them.
...
def test_scope_usage_reports_untouched_paths(tmp_path):
    ...
    result = run["result"]
    usage = result.scope_usage()
    # The test files are authorized but the receipt only changes
    # company/engineering/verify.py.
    assert "tests/test_company_engineering_execution.py" in usage.untouched_paths
    assert "tests/test_company_review_separation.py" in usage.untouched_paths
