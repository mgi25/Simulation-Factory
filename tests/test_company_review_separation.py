...
def test_a_future_job_completes_without_the_ceo(policy, config):
    ...
    Every delegated step answered below the CEO. This is the proof that the
    organizational defect is fixed rather than described.
    """
    order = _intake(
    ...
    chain = [
        (ActionType.APPROVE_CODE_CHANGE, "engineering_manager"),
        (ActionType.APPROVE_TEST_PROGRESSION, "engineering_manager"),
        (ActionType.APPROVE_REVIEW_OUTCOME, "engineering_manager"),
        (ActionType.APPROVE_INTEGRATION_MERGE, "engineering_manager"),
    ]
    actors = {}
    for action, seat in chain:
        decision = evaluate(
            _request(
                action,
                seat,
                reviewer=reviewer,
                implementer=implementer,
                risk=Risk.LOW if action is not ActionType.APPROVE_INTEGRATION_MERGE
                else Risk.MEDIUM,
