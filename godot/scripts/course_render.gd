...
## Offline renderer for the sloped course. Stills or a short clip.
##
## A sibling of `lab_render.gd` and of the production `offline_render.gd`, and
## a change to neither: the prior lab's frames stay reproducible and the replay
## renderer stays untouched. Only the scene it drives differs.
##
...
## that crawls and one that flies produce identical images.
##
## Usage, after `--`:
##
...
##     --shot=NAME            the lens for a clip
##     --no-glow=1            bloom off, for the control frame
##     --layout=a|b|c         which layout table to build
##     --detail=block|hero    blockout volumes or the authored modules

const CourseScene := preload("res://scripts/course_scene.gd")
...
func _build_viewport() -> void:
	...
	_scene = CourseScene.new()
	_scene.name = "CourseScene"
...
func _report_triangles() -> void:
	## Rough triangle count for the whole scene, walked once after warm-up.
	##
	## Indices are summed from every surface of every MeshInstance3D, so an
	## instance drawn twice counts twice - which is the number that matters
	## for a frame budget rather than the number of unique triangles authored.
	var total := 0
	var meshes := 0
	...
	while not stack.is_empty():
		...
		if node is MeshInstance3D and (node as MeshInstance3D).mesh != null:
			...
			for surface in mesh.get_surface_count():
				...
				if arrays.is_empty():
					continue
				# A surface may be indexed or not, and a primitive mesh can
				# report neither until it has been rasterised once - so both
				# slots are checked and a missing one simply contributes zero.
...
func _fail(message: String) -> void:
	push_error("course render failed: %s" % message)
	get_tree().quit(1)
