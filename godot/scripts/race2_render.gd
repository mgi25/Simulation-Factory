...
## Offline renderer for Race #2. Stills, a contact sheet, or every frame.
##
## A sibling of `sloped_race_render.gd` and a change to none of it: Race #1's
## frames keep reproducing byte for byte from this branch. Only the scene it
## drives differs, and only in that the course is built from the physics'
## own collider rather than from a visual twin.
##
## What it keeps is the part that matters: an offscreen `SubViewport` at the
## true output resolution, a fixed warm-up count, and a clock that is the
## *output frame index* rather than wall time. A render that crawls and one
## that flies produce identical images, which is the only reason a
## thousand-frame clip can be trusted to match a still.
##
## Usage, after `--`:
##
...
##     --at=1.5,2.0,...     one frame per second named, for a sheet
...
##     --environment=ID --contrast=v21 --racers=meridian --show=all

const RaceScene := preload("res://scripts/race2_scene.gd")
...
func _fail(message: String) -> void:
	push_error("race2 render failed: %s" % message)
	get_tree().quit(1)
