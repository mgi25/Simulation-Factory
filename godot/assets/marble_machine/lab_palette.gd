...
func get_material(key: String) -> StandardMaterial3D:
	...
	# The environment profile lands last, and only over the surfaces it
	# actually names. See `environment_builder.apply_palette`.
	#
	# Last is not "wins": the two tables are meant to be disjoint. A profile
	# names world surfaces, a machine pass names machine surfaces, and
	# `tests/test_sloped_v23_integration.py` fails if a shipped pair ever
	# names the same key -- that overlap is the bug, not the ordering.
