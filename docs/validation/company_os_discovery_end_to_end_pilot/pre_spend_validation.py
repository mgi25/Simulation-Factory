...
plan = PlanningEnvelope(
    objective_id="obj-discovery-e2e-pilot", budget=Money("6.00", "USD"),
...
env = PilotEnvelope(
    envelope_id="pilot-envelope-discovery-e2e", objective=OBJECTIVE, plan=plan,
...
act = PilotActivation(activation_id="pilot-activation-discovery-e2e", envelope=env,
...
def live(action, seat, risk, **kw):
    ...
    req = AuthorityRequest(
        ...
        department="engineering", risk=risk, objective_id="obj-discovery-e2e-pilot",
...
import pathlib
pathlib.Path("docs/evidence/company_os_discovery_end_to_end_pilot/pre_spend_validation.json").write_text(
