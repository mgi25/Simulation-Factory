...
## Correction: the management layer was not bypassed, the simulation was wrong

The first `future_job_simulation.json` recorded `cto` as the actor for all four
of `approve_code_change`, `approve_test_progression`, `approve_review_outcome`
and `approve_integration_merge`, which reads like the Engineering Manager being
bypassed. It is not. Two separate things produced that result, and neither is a
defect in the policy.

**The simulation filed every request as the manager.**
`test_a_future_job_completes_without_the_ceo` passed
`requesting_seat="engineering_manager"` for all four actions.
`company/delegation/authority.py` refuses self-approval — a seat that raises a
request is skipped when the chain is walked, which is separation of duties
doing its job — so the manager was excluded by construction from the three
decisions it is supposed to own, and each one fell through to the CTO. The test
asserted only that *integration* was decided by the CTO, so the three that
should have stopped at the manager slipped past unnoticed and were written into
the evidence file.

Fixed by filing each request from the seat that actually raises it: the
developer raises the code change and the test progression, the reviewer raises
the review outcome and any bounded correction, and only integration is raised
by the manager. Every decision is now asserted against its expected actor
rather than only the last one. With that, the manager takes four management
decisions and the CTO takes integration, with the CEO required nowhere.

**The manager's ceiling is `low`, on purpose.** At MEDIUM risk the manager
returns `risk_above_ceiling` and the CTO decides. That is the grant this policy
already declares — "routine low-risk engineering work" — against a CTO carrying
`medium`. It was not widened to make the chart symmetrical. An ordinary routine
engineering job is LOW risk and stops at the manager; a MEDIUM-risk one
reaching the CTO is the ladder working as written.

Neither change touches the architecture case. Work the routing calls
architectural still routes its review to `chief_architect`, the CTO is still
disqualified from integrating work its own employee reviewed, and that chain
still reaches the CEO. `test_architecture_review_still_disqualifies_the_cto_from_integration`
pins it.
